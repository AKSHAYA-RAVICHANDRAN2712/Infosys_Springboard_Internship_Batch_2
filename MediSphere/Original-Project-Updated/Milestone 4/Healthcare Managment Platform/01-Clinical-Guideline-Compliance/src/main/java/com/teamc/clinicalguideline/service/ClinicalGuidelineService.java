...
public class ClinicalGuidelineService {
    ...
    public ComplianceResponse check(ClinicalGuidelineRequest r){
        ...
        jdbc.update("insert into compliance_result(patient_id,care_plan_id,guideline_code,status,violations,warnings,checked_at) values (?,?,?,?,?,?,now())",
                r.getPatientId()==null?null:r.getPatientId().trim(),parseLong(r.getCarePlanId()),guidelineCode,status,String.join(" | ",violations),String.join(" | ",warnings));
    ...
    public List<Map<String,Object>> recentResults(){return jdbc.queryForList("select patient_id, care_plan_id, guideline_code, status, violations, warnings, checked_at from compliance_result order by id desc limit 20");}
