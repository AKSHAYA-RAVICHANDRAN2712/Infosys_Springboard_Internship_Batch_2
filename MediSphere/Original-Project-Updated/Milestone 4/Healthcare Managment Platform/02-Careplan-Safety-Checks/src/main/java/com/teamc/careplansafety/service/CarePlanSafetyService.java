...
public class CarePlanSafetyService {
 ...
 public CarePlanSafetyResponse validate(CarePlanSafetyRequest r){
  ...
  jdbc.update("insert into careplan_safety_result(patient_id,medication,dosage,unit,frequency,duration,status,violations,checked_at) values (?,?,?,?,?,?,?,?,now())",r.getPatientId().trim(),r.getMedication().trim(),r.getDosage(),r.getUnit().trim(),r.getFrequency().trim(),r.getDuration(),status,v.stream().map(SafetyViolation::getMessage).reduce((a,b)->a+" | "+b).orElse(""));
  return new CarePlanSafetyResponse(safe,status,v,OffsetDateTime.now().toString());
 }
 public List<Map<String,Object>> history(String patientId){return jdbc.queryForList("select id, medication, dosage, unit, frequency, duration, status, violations, checked_at from careplan_safety_result where patient_id=? order by id desc limit 20",patientId.trim());}
 public List<Map<String,Object>> recentResults(){return jdbc.queryForList("select patient_id, medication, dosage, unit, frequency, duration, status, violations, checked_at from careplan_safety_result order by id desc limit 20");}
