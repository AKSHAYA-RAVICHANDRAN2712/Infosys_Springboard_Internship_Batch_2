...
A merged, single coherent application built from four milestones:
...
- **Milestone 4** (`medisphere-integrated-platform`): Precision Care
  Management — AI-generated personalized care plans, outcome measurement,
  provider collaboration and clinical guideline compliance. Its frontend
  arrived as a static UI mockup (hardcoded numbers, no backend); the real
  backend logic (`PrecisionCareController`/`PrecisionCareService`, the
  `pc_*` tables) was written from scratch in this integration and the
  frontend was rewired to call it instead of using hardcoded data.

Milestone 1 is the base application. Milestone 2's, Milestone 3's, and
Milestone 4's real, working backend logic (not their mock UI screens or,
for Milestone 3, its own standalone frontend) has been integrated into it
as internal microservices (Milestones 2 and 3) or as a new module of the
main Spring Boot backend (Milestone 4).
...
React Frontend  (frontend/, Vite + React 18, port 5173 dev / 3000 docker)
      ...
      |         precision care plans · outcomes · guidelines · collaboration (M4)
      |
...
├── frontend/            React + Vite app — Milestone 1 base + Milestone 2/3/4 pages
...
│                        — Milestone 4's pc_* tables live in the Java
│                        backend instead (see "Milestone 4" below)
...
**From Milestone 4 (integrated as "Precision Care", ADMIN/DOCTOR only):**
Backed by a new module of the *same* Java backend used by Milestone 1
(`PrecisionCareController` / `PrecisionCareService`, tables prefixed
`pc_*`, created automatically by Hibernate and seeded in `data.sql` for
patient 102 / Arjun Verma) — not a mock. Includes a patient selector so it
works for any patient with a plan on file, not just the seeded demo.
- **Personalized Care Plan** — an AI-generated intervention plan with
  per-goal interventions/monitoring instructions; approve it or mark it
  sent to the patient.
- **Outcome Measurement** — tracked clinical metrics (HbA1c, blood
  pressure, CVD risk, adherence) against baseline/target, a CVD-risk
  trend chart rendered from real stored trend points (not a fixed SVG
  path), and careplan milestones.
- **Clinical Guideline Compliance** — guideline-by-guideline compliance
  status (ADA, ACC/AHA, CMS, USPSTF, Joint Commission) with a "Review
  now" action that flips a guideline to compliant.
- **Provider Collaboration** — the assigned care team, a shared activity
  feed you can actually post to, and a task list you can check off.
...
New in this integration — `/precision-care` (Milestone 4, ADMIN/DOCTOR only):

| Method | Path | Purpose |
|---|---|---|
| GET | `/precision-care/summary` | KPI aggregate (patients assessed, at-risk cohort, avg. prediction confidence/adherence/hospitalization-risk-reduction) across every plan |
| GET | `/precision-care/careplan/{patientId}` | `{ plan, patientName, goals }` for that patient's active plan |
| POST | `/precision-care/careplan/{patientId}/approve` | Mark the plan `Approved` |
| POST | `/precision-care/careplan/{patientId}/send` | Mark the plan `Sent to patient` |
| GET | `/precision-care/outcomes/{patientId}` | `{ metrics, cvdRiskTrend, milestones }` |
| GET | `/precision-care/guidelines/{patientId}?status=` | Guideline compliance rows, optionally filtered (`compliant` / `review`) |
| PATCH | `/precision-care/guidelines/{id}/review` | Mark a guideline reviewed → `compliant` |
| GET | `/precision-care/team/{patientId}` | Assigned care team members |
| GET | `/precision-care/activity/{patientId}` | Shared activity feed (newest first) |
| POST | `/precision-care/activity/{patientId}` | Post a new activity message |
| GET | `/precision-care/tasks/{patientId}` | Care-team task list |
| PATCH | `/precision-care/tasks/{id}/toggle` | Toggle a task done/not done |

Example:

```bash
curl http://localhost:8080/api/precision-care/summary -H "Authorization: Bearer <jwt>"
curl http://localhost:8080/api/precision-care/careplan/102 -H "Authorization: Bearer <jwt>"
curl -X POST http://localhost:8080/api/precision-care/careplan/102/approve -H "Authorization: Bearer <jwt>"
```
...
- **The Spring Boot backend could not be built or run in this sandbox** —
  ...
  `mvn clean package` cannot download dependencies. Everything under
  `backend/` from the original Milestone 1–3 integration is unmodified and
  was previously reviewed as described below. **The Milestone 4 addition
  (`entity`/`repository`/`service`/`controller` classes for Precision
  Care, the `SecurityConfig` rule, and the `pc_*` seed rows in
  `data.sql`) was written in this session and could not be compiled or
  run here either** — it was checked by hand against the same
  conventions the existing `CarePlan`/`Patient` vertical slice uses
  (Lombok `@Data` entities, `JpaRepository`, constructor-injected
  `@Service`, thin `@RestController`, `ResourceNotFoundException` for
  404s). Run `mvn clean package && mvn spring-boot:run` locally — on
  first boot Hibernate creates the `pc_*` tables (`ddl-auto=update`) and
  `data.sql` seeds patient 102's demo plan — and hit the endpoints in
  the table above (or open **Precision Care** in the sidebar as
  `admin@medisphere.com` / `doctor@medisphere.com`) to verify end-to-end.
