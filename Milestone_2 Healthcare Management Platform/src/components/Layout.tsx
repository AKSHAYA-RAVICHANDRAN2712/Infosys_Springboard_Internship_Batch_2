...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const PAGE_PERMISSIONS: Record<string, string[]> = {
      ...
      '/federated-training': ['admin', 'doctor', 'employee'],
      '/cvd-risk': ['admin', 'doctor', 'employee'],
      '/diabetes-risk': ['admin', 'doctor', 'employee'],
    ...
    if (allowedRoles && !allowedRoles.includes(currentUser.role)) {
      MediToast.error(
        `Access Denied: Your role (${currentUser.role.toUpperCase()}) cannot access ${location.pathname}`,
